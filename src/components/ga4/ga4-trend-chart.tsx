...
import type { Ga4TrendPoint } from "@/lib/ga4-analytics";
...
function Ga4ChartTooltip({
  active,
  payload,
}: {
  active?: boolean;
  payload?: Array<{ payload?: Ga4TrendPoint }>;
}) {
  ...
  return (
    <div className="rounded-xl border border-slate-200/80 bg-white/95 px-3.5 py-3 shadow-xl backdrop-blur">
      ...
      <div className="mt-2 grid grid-cols-2 gap-x-5 gap-y-1 text-[11px]">
        ...
        <span className="text-right font-semibold tabular-nums">{point.sessions.toLocaleString("pt-BR")}</span>
        <span className="text-slate-500">Usuários ativos</span>
        <span className="text-right font-semibold tabular-nums">{point.activeUsers.toLocaleString("pt-BR")}</span>
        ...
        <ComposedChart data={data} margin={{ top: 14, right: 8, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="ga4Sessions" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#47cdd0" stopOpacity={0.28} />
          ...
          <YAxis
            ...
            width={38}
          />
          <Tooltip content={<Ga4ChartTooltip />} cursor={{ stroke: "rgba(71,205,208,0.25)" }} />
          <Area
            yAxisId="volume"
            type="monotone"
            dataKey="sessions"
            ...
            dot={false}
            activeDot={{ r: 4, strokeWidth: 0, fill: "#47cdd0" }}
          />
          <Line
            yAxisId="rate"
            type="monotone"
