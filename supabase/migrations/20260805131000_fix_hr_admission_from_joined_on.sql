-- Corrige admissões inventadas (users.created_at) com joined_on do perfil NFC
-- e passa o sync VIOS a preferir essa data.

with fixed as (
  update public.hr_employees e
  set
    admission_date = p.joined_on,
    updated_at = now()
  from public.professional_profiles p
  where p.user_id = e.user_id
    and p.joined_on is not null
    and e.admission_date is distinct from p.joined_on
  returning e.id
)
delete from public.vacation_periods vp
where vp.employee_id in (select id from fixed);
