...
- O painel lateral (Sheet) mostra data, área, formato, situação, responsável com avatar, origem importada, Conteúdo, VIOS e publicação/métricas. Estados vazios distinguem `Tema ainda não escolhido` de `Não vinculado ao VIOS`.
- Marketing, administradores e gestores autorizados podem trocar o responsável enquanto o slot não possui conteúdo vinculado. As opções ficam restritas a colaboradores ativos e compatíveis com a área; slots vinculados explicam por que a troca está bloqueada.
...
- Suíte completa: 1.039 testes aprovados; um teste de IA ao vivo ignorado pela configuração existente; nenhuma falha.
...
- A validação visual autenticada do calendário, popover diário, Sheet e central de responsáveis será realizada separadamente pelo controlador.
